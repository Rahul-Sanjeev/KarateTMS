...
function AppShell() {
  ...
  const guardedSetTab = (tab) => {
    ...
    if (allowed === null) { setActiveTab(tab); return; }
  ...
  const renderPage = () => {
    if (activeTab === 'Login' || (!currentUser && activeTab !== 'Home' && activeTab !== 'Timer')) {
    ...
    if (activeTab !== 'Home' && activeTab !== 'Timer') {
      ...
      if (allowed !== null && (!currentUser || (allowed && !allowed.includes(currentUser.role)))) {
        return <Login setActiveTab={guardedSetTab} />;
      }
    }

    switch (activeTab) {
      case 'Home': return <Home setActiveTab={guardedSetTab} />;
      case 'Timer': return <Timer setActiveTab={guardedSetTab} />;
      case 'Dashboard': return <Dashboard setActiveTab={guardedSetTab} />;
      case 'Settings': return <Settings />;
      case 'Categories': return <Categories setActiveTab={guardedSetTab} />;
      case 'My Participants': return <Participants />;
      case 'Brackets': return <Brackets />;
      case 'Match Console': return <MatchConsole />;
      case 'Club Standings': return <ClubStandings />;
      default: return <Login setActiveTab={guardedSetTab} />;
    }
  };
